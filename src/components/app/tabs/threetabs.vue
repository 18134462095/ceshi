<template>
  <div class="commTabs">
    <van-tabs
      ...
      duration="0.15"
    >
      <index-filter ref="filter" @change="change"></index-filter>
      <van-tab
            ...
            >
              ...
              >
                <div class="nodata" v-if="lengths == 0">
                  <nodata :fun="onRefresh" :index="index"></nodata>
                </div>
                <index-list
                  v-else-if="loadData[index] && loadData[index].length"
                  :list="loadData[index]"
                  :key="i[name] + key"
                ></index-list>
...
export default {
  ...
  methods: {
    change(data) {
      // console.log(data);
      this.sort = Object.assign({}, data);
      this.onRefresh(this.active);
    },
    goodsLoad(index) {
      this.$parent.apis(index).then((res) => {
